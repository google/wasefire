...
//! Tests that Ed25519 is working properly.

#![no_std]
wasefire::applet!();

use wasefire::crypto::ed25519::{Private, Public, is_supported};
...
fn test_random() {
    debug!("test_random(): Computes Ed25519 with random private keys.");
    for _ in 0 .. 5 {
        let key = Private::generate().unwrap();
        let pubkey = key.public();
        let key = Private::import(&key.export().unwrap()).unwrap();
...
fn test(vectors: &[Vector]) {
    ...
    for &Vector { name, private, public, message, signature } in vectors {
        ...
        let mut a = [0; 32];
        private.public().unwrap().export(&mut a).unwrap();
        assert_eq!(&a, public);
        let public = Public::import(public).unwrap();
        let r = signature[.. 32].try_into().unwrap();
        let s = signature[32 ..].try_into().unwrap();
        assert!(public.verify(message, r, s).unwrap());
    }
}

pub struct Vector {
    pub name: &'static str,
    pub private: &'static [u8; 32],
    pub public: &'static [u8; 32],
    pub message: &'static [u8],
    pub signature: &'static [u8; 64],
}

// Those test vectors are taken from: https://datatracker.ietf.org/doc/html/rfc8032
