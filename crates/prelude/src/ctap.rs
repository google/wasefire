...
//! Provides an API for CTAP HID interface.
...
/// Provides high-level CtapHid API from low-level API.
///
/// This trait should only be implemented by the prelude and is thus sealed. Its purpose is to
/// provide a unique interface to the different CtapHids.
#[sealed(pub(crate))]
pub trait CtapHid {
    ...
    fn read(&self, buffer: &mut [u8; 64]) -> Result<usize, Error>;
    ...
    fn write(&self, buffer: &[u8; 64]) -> Result<usize, Error>;

    /// Flushes the CtapHid (in case reads or writes are buffered).
...
/// Returns how many bytes were read (and thus written to the buffer). This function does not
/// block, so if there are no data available for read, zero is returned.
pub fn read<T: CtapHid>(ctap_hid: &T, buffer: &mut [u8]) -> Result<usize, Error> {
    ctap_hid.read(buffer)
}

/// Synchronously reads at least one byte from a CtapHid into a buffer.
///
/// This function will block if necessary.
pub fn read_any<T: CtapHid>(ctap_hid: &T, buffer: &mut [u8]) -> Result<usize, Error> {
    let mut reader = Reader::new(ctap_hid, buffer);
...
///
/// This function will block if necessary.
pub fn read_all<T: CtapHid>(ctap_hid: &T, buffer: &mut [u8]) -> Result<(), Error> {
    let mut reader = Reader::new(ctap_hid, buffer);
...
pub fn read_byte<T: CtapHid>(ctap_hid: &T) -> Result<u8, Error> {
    let mut byte = 0;
    read_any(ctap_hid, core::slice::from_mut(&mut byte))?;
    Ok(byte)
}
...
/// Returns how many bytes were written (and thus read from the buffer). This function does not
/// block, so if the CtapHid is not ready for write, zero is returned.
pub fn write<T: CtapHid>(ctap_hid: &T, buffer: &[u8]) -> Result<usize, Error> {
    ctap_hid.write(buffer)
}

/// Writes at least one byte from a buffer to a CtapHid.
///
/// This function will block if necessary.
pub fn write_any<T: CtapHid>(ctap_hid: &T, buffer: &[u8]) -> Result<usize, Error> {
    let mut writer = Writer::new(ctap_hid, buffer);
...
///
/// This function will block if necessary.
pub fn write_all<T: CtapHid>(ctap_hid: &T, buffer: &[u8]) -> Result<(), Error> {
    let mut writer = Writer::new(ctap_hid, buffer);
...
/// Flushes the CtapHid (in case reads or writes are buffered).
pub fn flush<T: CtapHid>(ctap_hid: &T) -> Result<(), Error> {
    ctap_hid.flush()
}

/// Asynchronously listens for event notifications.
pub struct Listener<'a, T: CtapHid> {
    ctap_hid: &'a T,
    event: Event,
    notified: &'static Cell<bool>,
}

impl<'a, T: CtapHid> Listener<'a, T> {
    /// Starts listening for the provided event until dropped.
    pub fn new(ctap_hid: &'a T, event: Event) -> Self {
...
impl<'a, T: CtapHid> Drop for Listener<'a, T> {
    fn drop(&mut self) {
...
pub struct DelimitedReader<'a, T: CtapHid> {
...
impl<'a, T: CtapHid> DelimitedReader<'a, T> {
    /// Starts reading delimited frames from a CtapHid.
    pub fn new(ctap_hid: &'a T, delimiter: u8) -> Self {
...
#[must_use]
pub struct Reader<'a, T: CtapHid>(Updater<'a, T>);

impl<'a, T: CtapHid> Reader<'a, T> {
    /// Asynchronously reads from a CtapHid into a buffer.
    pub fn new(ctap_hid: &'a T, buffer: &'a mut [u8]) -> Self {
...
#[must_use]
pub struct Writer<'a, T: CtapHid>(Updater<'a, T>);

impl<'a, T: CtapHid> Writer<'a, T> {
    /// Asynchronously writes from a buffer to a CtapHid.
    pub fn new(ctap_hid: &'a T, buffer: &'a [u8]) -> Self {
...
impl<'a, T: CtapHid> Updater<'a, T> {
    fn new(ctap_hid: &'a T, kind: Kind<'a>) -> Self {
...
impl<'a> Kind<'a> {
    ...
    fn update(&mut self, ctap_hid: &impl CtapHid, pos: usize) -> Result<usize, Error> {
        match self {
