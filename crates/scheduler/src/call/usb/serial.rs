...
use ::api::usb::serial::{self as api, Api};
use board::usb::serial::{Api as Bpi, Event};
...
fn read<B: Board>(mut call: SchedulerCall<B, api::read::Sig>) {
    ...
    let results = try {
        ...
        let len = match <B as Bpi>::read(&mut scheduler.board, output) {
...
fn write<B: Board>(mut call: SchedulerCall<B, api::write::Sig>) {
    ...
    let results = try {
        ...
        let len = match <B as Bpi>::write(&mut scheduler.board, input) {
            Ok(len) => (len as u32).into(),
            Err(_) => u32::MAX.into(),
        };
...
fn register<B: Board>(mut call: SchedulerCall<B, api::register::Sig>) {
    ...
    let results = try {
        ...
        <B as Bpi>::enable(&mut scheduler.board, &event).map_err(|_| Trap)?;
...
fn unregister<B: Board>(mut call: SchedulerCall<B, api::unregister::Sig>) {
    ...
    let results = try {
        let event = convert_event(*event)?;
        <B as Bpi>::disable(&mut scheduler.board, &event).map_err(|_| Trap)?;
...
fn flush<B: Board>(mut call: SchedulerCall<B, api::flush::Sig>) {
    ...
    let scheduler = call.scheduler();
    let results = try {
        let res = match <B as Bpi>::flush(&mut scheduler.board) {
            Ok(()) => 0.into(),
            Err(_) => u32::MAX.into(),
        };
