...
impl<'a> Fuzzer<'a> {
    ...
    fn new(data: &'a [u8], debug: bool, stats: Option<&'a mut Stats>) -> Fuzzer<'a> {
        ...
        let mut fuzzer =
            Fuzzer { entropy, values, init: Init::Clean, debug, stats, counters: HashMap::new() };
    ...
    fn init(&mut self) -> StoreDriver {
        ...
        if self.entropy.read_bit() {
            ...
            if self.debug {
                println!("Start with {cycle} consumed erase cycles.");
            }
    ...
    fn operation(&mut self, driver: &StoreDriverOn) -> StoreOperation {
        ...
        match self.entropy.read_range(0, 2) {
            0 => {
                ...
                let count = self.entropy.read_range(0, format.max_updates() as usize + 1);
            ...
            2 => {
                ...
                let length = self.entropy.read_range(0, format.total_capacity() as usize + 1);
    ...
    fn interruption(&mut self, max_delay: Option<usize>) -> StoreInterruption<'_> {
        ...
        if self.debug {
            if delay == max_delay {
                ...
                println!("Interrupt after {delay} operations.");
            }
        }
        if delay < max_delay {
        ...
        let corrupt = Box::new(move |old: &mut [u8], new: &[u8]| {
            ...
            if self.debug {
                println!("Flip {count} bits out of {total}.");
            }
        });
